...
export default function AddDoctor() {
  ...
  useEffect(() => {
    if (param.id) {
      debugger
      const fetchApi = async () => {
        const result = await getUserById(Number(param.id));
        if (result) {
          form.setFieldsValue({
            ...result.data,
  ...
  const onFinish = async (values) => {
    ...
    if (param.id) {
      values.id = Number(param.id);
    }
    if (values.major && typeof values.major === "object") {
      values.major = Number(values.major);
    }
  ...
  return (
    <>
      <Space direction="vertical" size="middle" style={{ display: "flex" }}>
        <h1>{param.id ? "Sửa thông tin bác sĩ" : "Thêm mới bác sĩ"}</h1>
        {formAddDoctor}
      </Space>
    </>
  );
