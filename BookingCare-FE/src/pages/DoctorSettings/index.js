...
export default function DoctorSetting() {
  ...
  const profileUserID = JSON.parse(localStorage.getItem("ID"));
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await getUserById(profileUserID);
        const data = response.data;

        // Ensure dateOfBirth is valid and parse it correctly
        const formattedDate = data.dateOfBirth
            ? moment(data.dateOfBirth, "YYYY-MM-DD").isValid()
